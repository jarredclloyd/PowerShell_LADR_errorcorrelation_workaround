...
For decay system:
'RbSr', 'Rb85 -> 85' will be replaced by 'U238', 'Sr87 -> 103' will be replaced by 'Pb207, and 'Sr86 -> 102' will be replaced by 'Pb206'. Additonally 'U238 -> 270' will be replaced by 'U235'.
'LuHf', 'Lu175 -> 175' will be replaced by 'U238', 'Hf176 -> 258' will be replaced by 'Pb207, and 'Hf178 -> 260' will be replaced by 'Pb206'. Additonally 'U238 -> 270' will be replaced by 'U235'.
    ...
    switch ($decaysystem) {
            'RbSr' { if (Test-Path $RbSrtoUPbDir) {
                ...
                Copy-Item -Path $originalsdircsv -Destination $RbSrtoUPbDir
                    Get-ChildItem -Path $RbSrtoUPbDir| ForEach-Object -ThrottleLimit 16 -Parallel {
                        $outfile = $_.FullName 
                        [io.file]::ReadAllText($_.FullName).replace('Rb85 -> 85', 'U238').replace('Sr87 -> 103', 'Pb207').replace('Sr86 -> 102', 'Pb206').replace('U238 -> 270', 'U235') |
                        Out-File $outfile
                    }
            ...
            'LuHfNorm' { if (Test-Path $LuHfNormtoUPbDir) {
                    ...
                    Get-ChildItem -Path $LuHfNormtoUPbDir | ForEach-Object -ThrottleLimit 16 -Parallel {
                        $outfile = $_.FullName 
                        [io.file]::ReadAllText($_.FullName).replace('Lu175 -> 175', 'U238').replace('Hf176 -> 258', 'Pb207').replace('Hf178 -> 260', 'Pb206').replace('U238 -> 270', 'U235') |
                        Out-File $outfile
                    }
                    Write-Host 'Task completed.'
                }
            ...
            'LuHfInv' {
                if (Test-Path $LuHfInvtoUPbDir) {
                    ...
                    Get-ChildItem -Path $LuHfInvtoUPbDir | ForEach-Object -ThrottleLimit 16 -Parallel {
                        $outfile = $_.FullName 
                        [io.file]::ReadAllText($_.FullName).replace('Lu175 -> 175', 'U238').replace('Hf176 -> 258', 'Pb206').replace('Hf178 -> 260', 'Pb207').replace('U238 -> 270', 'U235') |
                            Out-File $outfile
                        }
                        Write-Host 'Task completed.'
                    }
